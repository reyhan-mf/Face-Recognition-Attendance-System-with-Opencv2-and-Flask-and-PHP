...
$image = mysqli_query($conn, "SELECT student_picture FROM SISWA WHERE user_id = $user_id");
if ($row = mysqli_fetch_assoc($image)) {
  $student_picture = $row['student_picture'];
} else {
  $student_picture = 'default.jpg'; // Path to a default image if no picture is found
}
?>
<!DOCTYPE html>
<html lang="en">
  ...
  <script>
    ...
    function validateForm() {
      ...
      const kelas = document
        .getElementById("dropdownMenuButtonKelas")
        .textContent.trim();
      const mataPelajaran = document
        .getElementById("dropdownMenuButtonMataPelajaran")
        .textContent.trim();
      const pertemuan = document
        .getElementById("dropdownMenuButtonPertemuan")
        .textContent.trim();
    ...
    function toggleEditMode() {
      ...
      if (!editMode) {
        ...
        originalStatuses = Array.from(statusSelects).map(
          (select) => select.value
        );
      }
    ...
    function saveChanges() {
      ...
      statusSelects.forEach((select, index) => {
        ...
        statusDisplays[
          index
        ].className = `status-display badge badge-${getBadgeClass(newValue)}`;
      });
                    ...
                    <table class="table table-hover">
                      ...
                      <tbody>
                        <tr>
                          ...
                          <td>
                            <a href="">MATEMATIKA</a>
                            <img src="data:image/jpeg;base64,<?php echo base64_encode($student_picture); ?>" alt="Student Picture" />
